...
def generate_fake_hdf5(output_path, num_samples=200):
    """
    Write fake teleoperation data into an HDF5 file so you can verify the
    logging pipeline works independently of live hardware.

    Pose format: [X, Y, Z, Rx, Ry, Rz]
      - X/Y/Z in metres  (RealMan RM65 typical workspace ~0.2–0.65 m radius)
      - Rx/Ry/Rz in radians (Euler angles, capped to ~±pi)

    MANUS ergonomics: 20 floats, joint angles in degrees (thumb→pinky).
    LEAP Hand pose:   16 floats, Dynamixel raw positions (allegro_to_LEAPhand units).
    """
    if h5py is None:
    ...
    # Simulate a trajectory: the arm moves slowly in XYZ + slight wrist rotation
    base_pose = np.array([0.35, 0.0, 0.45, 0.0, 0.0, 0.0], dtype=np.float64)
    ...
    with h5py.File(output_path, "w") as f:
        ...
        f.attrs["source"] = "generate_fake_hdf5"
        f.attrs["num_samples"] = num_samples
        f.attrs["robot_ip"] = "192.168.1.18"
        f.attrs["control_hz"] = 125.0
        f.attrs["note"] = "Fake data for HDF5 logging validation"

        # --- create resizable datasets (mirrors TeleopHDF5Logger) ---
        def mkds(name, shape, maxshape, dtype):
            return f.create_dataset(name, shape=shape, maxshape=maxshape,
                                    dtype=dtype, chunks=True)

        ds = {
            "time/monotonic_s":  mkds("time/monotonic_s",  (0,),      (None,),      np.float64),
            "time/wall_time_s":  mkds("time/wall_time_s",  (0,),      (None,),      np.float64),
            "arm/raw_pose":      mkds("arm/raw_pose",      (0, 6),    (None, 6),    np.float64),
            "arm/bounded_pose":  mkds("arm/bounded_pose",  (0, 6),    (None, 6),    np.float64),
            "arm/safe_pose":     mkds("arm/safe_pose",     (0, 6),    (None, 6),    np.float64),
            "arm/smoothed_pose": mkds("arm/smoothed_pose", (0, 6),    (None, 6),    np.float64),
            "arm/hold_flag":     mkds("arm/hold_flag",     (0,),      (None,),      np.bool_),
            "arm/canfd_status":  mkds("arm/canfd_status",  (0,),      (None,),      np.int32),
            "hand/manus_joints": mkds("hand/manus_joints", (0, 20),   (None, 20),   np.float64),
            "hand/leap_pose":    mkds("hand/leap_pose",    (0, 16),   (None, 16),   np.float64),
            "hand/has_glove_data": mkds("hand/has_glove_data", (0,),  (None,),      np.bool_),
        }

        interpolator = HighFrequencyInterpolator(alpha_pos=0.15, alpha_rot=0.15)

        for i in range(num_samples):
            dt_s = i / 125.0  # simulate 125 Hz

            # Drift the arm pose slowly around a home position
            t = dt_s
            raw_pose = base_pose + np.array([
                0.05 * np.sin(0.5 * t),
                0.03 * np.cos(0.7 * t),
                0.02 * np.sin(1.1 * t),
                0.10 * np.sin(0.3 * t),
            ...
            # Occasionally simulate a None raw_pose (boundary condition) for ~5 % of frames
            have_valid_pose = rng.random() > 0.05
            bounded_pose = raw_pose.copy() if have_valid_pose else None
            safe_pose = raw_pose.copy() if have_valid_pose else base_pose.copy()
            smoothed_pose = np.array(interpolator.step(safe_pose))
            hold_flag = not have_valid_pose

            # MANUS ergonomics: 20 joint angles in degrees, roughly 0-90
            manus_joints = rng.uniform(0, 60, size=20).tolist()

            # LEAP Hand pose: 16 Dynamixel positions
            leap_pose = rng.uniform(1500, 2500, size=16)

            values = {
                "time/monotonic_s":  t0 + dt_s,
                "time/wall_time_s":  wall0 + dt_s,
                "arm/raw_pose":      np.asarray(
                                         raw_pose if have_valid_pose else [np.nan] * 6,
                                         dtype=np.float64),
                "arm/bounded_pose":  np.asarray(
                                         bounded_pose if bounded_pose is not None else [np.nan] * 6,
                                         dtype=np.float64),
                "arm/safe_pose":     np.asarray(safe_pose, dtype=np.float64),
                "arm/smoothed_pose": np.asarray(smoothed_pose, dtype=np.float64),
                "arm/hold_flag":     bool(hold_flag),
                "arm/canfd_status":  int(0),
                "hand/manus_joints": np.asarray(manus_joints, dtype=np.float64),
                "hand/leap_pose":    np.asarray(leap_pose, dtype=np.float64),
                "hand/has_glove_data": True,
            }
        ...
        f.attrs["sample_count"] = num_samples
        f.flush()

    print(f"Fake HDF5 written: {output_path}  ({num_samples} samples)")
    print("Datasets:")
...
def build_parser():
    ...
    parser.add_argument(
        ...
        metavar="OUTPUT_PATH",
        default=None,
        help="Write fake teleoperation data to OUTPUT_PATH and exit "
             "(e.g. --test-hdf5 logs/fake_test.hdf5). "
             "Use this to verify HDF5 logging works without live hardware.",
    )
    return parser
...
def main():
    ...
    if args.test_hdf5 is not None:
        generate_fake_hdf5(args.test_hdf5)
        return
